...
def resource_usage(item, cost):
    """Deducts the resource used by the item."""
...
def resource_available(item):
    """Checks if resources are available for the item."""
...
def cost_of(item):
    """Calculates the cost of item, Calculates the change to be give (if any), Also verifies if enough money is entered"""
    cost = MENU[item]["cost"]
